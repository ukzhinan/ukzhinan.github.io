...
## 2026-09-19 内容扩展与维护约定

新增七个专题：落地第一周、eVisa、看病与 NHS、学术诚信与 AI、歧视骚扰与关系伤害求助、毕业求职实战、搬家与离英交接。全站现有 71 个 `_chapters` 文档，其中 56 个为 `section` 专题。

- 首页先展示行前、落地、在读、毕业和求助五条入口，作者介绍移至目录之后。
- 专题的 `action` 包含 `audience`、`prepare`、`steps`、`help` 四个非空字符串，由 `_includes/action-summary.html` 在目录前显示。行前和速查页也使用同一组件。摘要需说明适用地区或资格，不能把正文里的例外省略成绝对承诺。
- `updated` 显示为「内容编辑」，不代表整页政策同日核实。核对日期与来源放在具体段落；不要为增加摘要而更新旧税表的生效时点。
- 医疗流程主页面为 `/shenghuo/nhs-kanbing/`；旧饮食页继续保留 `sec-3` 至 `sec-6` 的医疗入口，维护时不能直接删掉旧锚点。
- eVisa 办事、学术诚信和搬家交接各有主页面。概览页保留简要解释并链接主页面，避免复制整套可变的规则或期限。
- 报修模板在租房页；就诊准备表在医疗页；延期邮件和小组贡献表保留在原学业页；搬家、AI 使用记录和求职跟踪表放在对应专题。例子不等于法定格式，虚构教学数据不能当作真实经历或研究。
- 购物先讲需求、设备限制、总价与退出成本，合作链接作为可选入口。优惠资格与商品参数仍需购买时自行确认。

范围、来源与复核边界见 [2026-09-19 内容扩展记录](docs/content-expansion-2026-09-19.md)。
